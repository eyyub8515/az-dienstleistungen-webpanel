...
function normalizeCustomerTypeValue(
  value?: string | null
): 'private' | 'business' {
...
function mapActor(actor?: RawRequestActor | null) {
  ...
  return {
    ...
    customerType: normalizeCustomerTypeValue(actor.customerType),
  };
}
...
function mapRequest(item: RawRequest): RequestItem {
  ...
  return {
    ...
    userId: item.userId
      ? {
          ...item.userId,
          customerType: normalizeCustomerTypeValue(item.userId.customerType),
        }
      : undefined,

    customerType: normalizeCustomerTypeValue(item.customerType),
